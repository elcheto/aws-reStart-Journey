Lab Report: Vulnerability Assessment with Amazon Inspector
Prepared by: Elena
...
Code Update: I navigated to the Lambda console and opened the get-request function.

Version Control: I modified the requirements.txt file, changing requests==2.20.0 to requests to ensure the latest secure version would be used.

Deployment: I deployed the changes, which automatically triggered a re-scan by Inspector.

Confirmation: I returned to the Inspector dashboard and confirmed the finding was moved to Closed status, verifying that the vulnerability was successfully resolved.
